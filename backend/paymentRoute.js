...
router.post('/callback', (req, res) => {
    ...
    form.parse(req, (err, fields, file) => {
        ...
        var isVerifySignature = PaytmChecksum.verifySignature(fields, merchantkey, paytmChecksum);
        if (isVerifySignature) {
            ...
            */
            PaytmChecksum.generateSignature(paytmParams, merchantkey).then(function (checksum) {
                    ...
                    post_res.on('end', function () {
                        ...
                        if (result.STATUS === 'TXN_SUCCESS') {
                            ...
                            MongoClient.connect(url, function(err, db) {
                                ...
                                var dbo = db.db("doctors_clone");
                                let dts = new Date(result.TXNDATE);
                                let update = dts.getFullYear() + '-' + (dts.getUTCMonth() > 10 ? dts.getUTCMonth() + 1 : '0' + (dts.getUTCMonth() + 1)) + '-' + (dts.getDate() > 10 ? dts.getDate() : '0' + dts.getDate());
                                var doc = { $set: { status:"Completed",txnId:result.TXNID,paymentMode:result.PAYMENTMODE,bankName:result.BANKNAME,gatewayName:result.GATEWAYNAME,bankTxnId:result.BANKTXNID,updatedDate:update } };
                                var myquery = { orderId:  result.ORDERID};
                                // insert document to 'users' collection using insertOne
                                dbo.collection("payments").updateOne(myquery, doc, function(err, res) {
                                    if (err) throw err;
                        ...
                        res.redirect(`http://localhost:3000/patient/status/${result.ORDERID}`)
                    });
                });
...
router.post('/payment', (req, res) => {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        dbo.collection("users").findOne({_id: ObjectId(doctorId)} , function(err, result) {
           if (err) throw err;
    ...
    params['CALLBACK_URL'] = 'http://localhost:5000/api/v1/payments/callback',
    params['EMAIL'] = email,
    ...
    */
    ...
    paytmChecksum.then(function (checksum) {
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            var doc = { patientId: patientId,patientName:name,patientPhone:phone, doctorId: doctorId, status:"Pending",orderId:orderId,amount:totalAmount };
