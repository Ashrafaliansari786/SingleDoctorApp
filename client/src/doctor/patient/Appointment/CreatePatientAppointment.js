...
const CreatePatientAppointment = () => {
  ...
  useEffect(async () => {
    ...
    await dispatch(doctorDetailById('626cd04a857ce8a353529632'));
  }, []);

  const { doctorDetails } = useSelector((state) => state.doctorDetails);

  //console.log('dewdwedfwef',doctorDetails);
  ...
  const [patientDetail, setPatientDetail] = useState({ patientId: user._id, patientName: user.name, patientEmail: user.email,phone:user.phone });
